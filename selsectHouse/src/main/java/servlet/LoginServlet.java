...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case "staffLogin":
			...
			break;
		case "purchaserLogin":
			purchaserLogin(request, response);
			break;
	...
	private void purchaserLogin(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		...
		if (purchaser.getPURCHASER_ID() == 0) {
			...
			session.setAttribute("userid", purchaser.getPURCHASER_ID());
	...
	private void purChaserVerifyInfo(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
	...
	private void staffLogin(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String name = MyUTF.getNewString(request.getParameter("txtXm"));
	...
	private void staffQuery(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		List<HouseSource> houseSources = houseSourceDao.selectInfoAll();
		// 把houseSources放入request集合中
		request.setAttribute("HouseSource", houseSources);
