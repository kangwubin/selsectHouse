...
//			System.out.println(method+"id: "+id);
			...
			switch(method){
				...
				case "adminQuery":  staffQuery(request,response);request.getRequestDispatcher("/afterView/guanliyuan-guanliyuancaozuo.jsp").forward(request,response) ;break;
	...
	private void staffAdd(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException  {
		...
		int privilege=Integer.parseInt(MyUTF.getNewString(request.getParameter("privilege"))) ;
		
		System.out.println("name: "+name+"id: "+id+"pwd: "+pwd+"privilege: "+privilege);
		boolean istrue	= staffDao.addInfo(id, name, pwd, privilege);
		if(istrue) {
			staffQuery(request,response);
			request.getRequestDispatcher("/afterView/guanliyuan-guanliyuancaozuo.jsp").forward(request,response) ;	
		}else
			request.getRequestDispatcher("/afterView/guanliyuan-guanliyuancaozuo_add.jsp").forward(request,response) ;
	...
	private void adminUpdate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException  {
		...
		String name=MyUTF.getNewString(request.getParameter("adminname")) ;
		...
		System.out.println("姓名："+name+"旧密码："+oldpwd+"新密码 ："+newpwd+"重复密码："+renewpwd);
		Staff staff	=	new Staff();
		staff	=	staffDao.login(name, oldpwd);
		if(newpwd.equals(renewpwd)) {
			...
			}else
			{
				...
				request.getRequestDispatcher("/afterView/guanliyuan-xiugaimima.jsp").forward(request,response) ;
			}
		}else
		{
